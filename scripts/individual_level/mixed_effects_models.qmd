...
# Convergence results
  ...
  data.frame(lapply(m5a.allfit_OK, function(x) x@optinfo$conv$lme4$messages)) |> 
    ...
    mutate(`Method` = if_else(lag(`Method`) != `Method` | is.na(lag(`Method`)), 
                              `Method`, ""),
           `Estimation result` = str_replace_all(`Estimation result`,
                                                 "|", " ")) |> 
    kable(booktabs = T, 
...
# Gradient 1
  ...
  max(abs(sc_grad1))
  kable(max(pmin(abs(sc_grad1), abs(derivs1$gradient))), 
        col.names = "Gradient, method 1", booktabs = T)
...
# Gradient 2
  ...
  kable(max(pmin(abs(sc_grad2), abs(grad2))), 
        col.names = "Gradient, method 2", booktabs = T)

  ## The gradient cutoffs for Nelder Mead (nlopt), bobyqa and nlminwrap seem to 
  ## be close to what I am getting. 

# Check for singular fit 
kable(isSingular(m5a.nested), col.names = "Singular fit?")
```
...
# Coefficients from different optimizers
  ...
  ## Plot
  ggplot(m5p.allfit.fixef.m[1:112, ], 
           aes(x = `Coefficient`, y = `Method`, colour = `Method`)) +
      geom_point() +
      facet_wrap(~ `Variable`, scale = "free") +
      scale_colour_brewer(palette = "Dark2") +
...
# Check for singular fit 
kable(isSingular(m5p.nested), col.names = "Singular fit?") # Nope, no singular fit
```
...
# Coefficients from different optimizers
  ...
  ## Plot
  ggplot(m5d.allfit.fixef.m[1:112, ], 
           aes(x = `Coefficient`, y = `Method`, colour = `Method`)) +
      geom_point() +
      facet_wrap(~ `Variable`, scale = "free") +
      scale_colour_brewer(palette = "Dark2") +
...
# Check for singular fit 
kable(isSingular(m5d.nested), col.names = "Singular fit?") # Nope, no singular fit
```
